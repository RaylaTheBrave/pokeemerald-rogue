...
SINGLE_BATTLE_TEST("Dragon's Maw turns a sound move into a Dragon-type move")
{
    u16 ability;
    PARAMETRIZE { ability = ABILITY_NONE; }
    PARAMETRIZE { ability = ABILITY_DRAGONSROAR; }

    GIVEN {
        PLAYER(SPECIES_TYPHLOSION);
        OPPONENT(SPECIES_NOIVERN) { Ability(ability); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_HYPER_VOICE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_HYPER_VOICE, opponent);
        if(ability == ABILITY_DRAGONSROAR)
